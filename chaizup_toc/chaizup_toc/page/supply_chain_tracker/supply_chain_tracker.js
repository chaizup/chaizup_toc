...
class SupplyChainTracker {
    ...
    if (stage === "items" || sub === "output") {
      ...
      switch (sub) {
        case "mr":
          ...
          if (node.required_date) rows += r("Due", fmtDate(node.required_date), node.is_overdue ? "err" : "warn");
        ...
        case "rfq":
          rows += r("Qty",   node.qty || "");
          if (node.transaction_date) rows += r("Date", fmtDate(node.transaction_date));
          break;
        case "pp":
          rows += r("Planned", node.planned_qty ? `${node.planned_qty}` : "");
          if (node.posting_date) rows += r("Date", fmtDate(node.posting_date));
          break;
        case "sq":
          ...
          break;
        case "wo":
          const wPct = node.progress_pct || 0;
          rows += r("Qty", `${node.produced_qty || 0} / ${node.qty || 0}`, wPct >= 100 ? "ok" : "mfg");
          if (node.planned_end_date) rows += r("End", fmtDate(node.planned_end_date), node.is_overdue ? "err" : "warn");
          break;
        case "po":
          rows += r("Qty",   node.qty || "");
          rows += r("Total", fmtMoney(node.grand_total));
          if (node.expected_delivery) rows += r("ETA", fmtDate(node.expected_delivery), node.is_overdue ? "err" : "");
          break;
        case "jc":
          const jPct = node.progress_pct || 0;
          rows += r("Done", `${node.completed_qty || 0} / ${node.for_quantity || 0}`, jPct >= 100 ? "ok" : "mfg");
          if (node.operation) rows += r("Op", node.operation);
          break;
        case "pr":
          rows += r("Recv", node.qty ? `${node.qty}` : "", "ok");
          if (node.rejected_qty > 0) rows += r("Rej", node.rejected_qty, "err");
          if (node.posting_date) rows += r("Date", fmtDate(node.posting_date));
          break;
        case "qi":
  ...
  _panelFields(node) {
    ...
    const fmtD = d => d ? frappe.datetime.str_to_user(d) : "";
    ...
    switch (node.sub_type || node.stage) {
      case "mr":
        ...
        add("Required By", fmtD(node.required_date));
        add("Warehouse",   node.warehouse);
        add("Recorded By", node.recorded_by || "Manual");
        add("Ref Date",    fmtD(node.transaction_date));
        break;
      case "rfq":
        add("Suppliers",  node.description);
        add("Status",     node.status);
        add("Qty",        node.qty);
        add("Date",       fmtD(node.transaction_date));
      ...
      case "pp":
        ...
        add("Post Date",   fmtD(node.posting_date));
        add("From MR",     node.mr_ref);
        add("Item Code",   node.item_code);
        break;
      case "sq":
      ...
      case "wo":
        ...
        add("Start Date",      fmtD(node.planned_start_date));
        add("End Date",        fmtD(node.planned_end_date));
        add("Actual Start",    fmtD(node.actual_start_date));
      ...
      case "po":
        ...
        add("Grand Total", fmt(node.grand_total));
        add("Expected",    fmtD(node.expected_delivery));
        add("Trans Date",  fmtD(node.transaction_date));
      ...
      case "jc":
        ...
        add("Progress",    `${node.progress_pct}%`);
        add("Expected End",fmtD(node.expected_end_date));
        add("Actual Start",fmtD(node.actual_start_date));
  ...
  _updateSummary(s, meta) {
    ...
    strip.innerHTML = [
      ...
      s.avg_bp_pct ? `<div class="sct-stat">
        <span class="sct-stat-val">${s.avg_bp_pct}%</span>
        <span class="sct-stat-lbl">📈 Avg BP%</span>
       </div>` : "",
    ].join("");
  }
  ...
  _statusBadgeClass(status) {
    ...
    const map = {
      ...
      "on-hold": "sct-badge-stopped", "closed": "sct-badge-completed",
      "accepted": "sct-badge-completed", "rejected": "sct-badge-stopped",
      "partially-received": "sct-badge-pending", "to-receive": "sct-badge-pending",
      "approved": "sct-badge-submitted", "rejected": "sct-badge-stopped",
    };
